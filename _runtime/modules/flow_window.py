...
from modules.clipboard_watcher import copy_image_to_clipboard
...
class FlowResultWindow:
    ...
    def __init__(self, parent, workspace, document, flow_path, status_callback=None, error_callback=None):
        self.parent = parent
        ...
        tk.Label(
            header,
            text="이미지나 텍스트 블록을 우클릭하면 해당 내용만 복사할 수 있습니다.",
            fg="#4b5563",
        ).pack(side="left")
    ...
    def _copy_image_path(self, image_path: Path) -> None:
        copy_image_to_clipboard(Path(image_path), owner_hwnd=int(self.window.winfo_id()))
        ...
        for section in sections:
            ...
            for item in section.get("items", []):
                ...
                if item_type == "capture":
                    self._add_image_block(section_frame, Path(str(item.get("imageSrc") or "")))
                elif item_type in {"explanation", "note"}:
                    self._add_text_block(section_frame, _plain(str(item.get("html") or "")))
                elif item_type == "code":
                    self._add_code_block(section_frame, str(item.get("code") or ""))
    ...
    def _add_image_block(self, parent, path: Path) -> None:
        block = tk.LabelFrame(parent, text="그림 · 우클릭하여 원본 복사", padx=8, pady=8, fg="#374151")
        block.pack(fill="x", pady=6)
        self._bind_context_menu(
            block,
            "원본 이미지 복사",
            lambda value=path: self.copy_image_block(value),
        )
        if not path.is_file():
    ...
    def _add_text_block(self, parent, value: str) -> None:
        block = tk.LabelFrame(parent, text="텍스트 · 우클릭하여 복사", padx=8, pady=8, fg="#374151")
        block.pack(fill="x", pady=6)
        self._bind_context_menu(block, "텍스트 복사", lambda text=value: self.copy_text_block(text))
        label = tk.Label(block, text=value, justify="left", anchor="w", wraplength=800, cursor="hand2")
        label.pack(fill="x", anchor="w")
        self._bind_context_menu(label, "텍스트 복사", lambda text=value: self.copy_text_block(text))

    def _add_code_block(self, parent, value: str) -> None:
        block = tk.LabelFrame(parent, text="텍스트 · 우클릭하여 복사", padx=8, pady=8, fg="#374151")
        block.pack(fill="x", pady=6)
        self._bind_context_menu(block, "텍스트 복사", lambda text=value: self.copy_text_block(text))
        ...
        code.pack(fill="x")
        self._bind_context_menu(code, "텍스트 복사", lambda text=value: self.copy_text_block(text))
