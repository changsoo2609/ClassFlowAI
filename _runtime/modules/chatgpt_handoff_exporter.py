...
- 코드에는 `pre`와 `code` 사용
- 제목 단계는 `h1 → h2 → h3` 유지
- 각 학습 단계에 관련 캡처 이미지를 실제 `img` 태그로 표시
- `img`의 `src`는 `images/capture_001.png`처럼 ZIP 내부 상대 경로 사용
- 이미지 바로 아래에 원본 파일명과 해당 화면이 보여주는 학습 근거를 설명
...
Notion 붙여넣기 결과는 아래 필수 규칙의 루트 파일 5개와 `images/` 폴더를 포함한
...
`images/`에는 각 학습 단계에서 사용한 입력 캡처 원본을 재인코딩하지 말고 그대로 복사하세요.
`notion_ready.html`과 `notion_ready.md`의 이미지 참조 순서는 사용자 학습 흐름 순서를 따라야 합니다.
...
def inline_local_images(html: str) -> str:
    ...
    def replace_source(match):
        nonlocal local_image_count
        source = match.group(3).replace("\\", "/")
        ...
        try:
            ...
            raise ValueError(f"Image path must stay inside images/: {source}") from exc
        if not image_path.is_file():
            raise FileNotFoundError(f"Referenced image is missing: {source}")
        mime = mimetypes.guess_type(image_path.name)[0] or "image/png"
        if not mime.startswith("image/"):
            raise ValueError(f"Referenced file is not an image: {source}")
        encoded = base64.b64encode(image_path.read_bytes()).decode("ascii")
        data_uri = f"data:{mime};base64,{encoded}"
        local_image_count += 1
        return f"{match.group(1)}{match.group(2)}{data_uri}{match.group(2)}"

    inlined = IMAGE_SRC_PATTERN.sub(replace_source, html)
    if local_image_count == 0:
        raise ValueError("notion_ready.html does not contain any packaged images")
    return inlined
...
def main() -> int:
    ...
    try:
        html = inline_local_images(HTML_PATH.read_text(encoding="utf-8"))
...
`notion_paste_package.zip`에는 다음 루트 파일 5개와 `images/` 폴더를 정확히 포함하세요.
...
`images/`에는 `notion_ready.html`과 `notion_ready.md`에서 실제로 사용하는 캡처 원본을
`capture_001.png` 같은 ASCII 파일명으로 포함하세요. 입력 이미지를 크기 축소, 재압축 또는
재인코딩하지 말고 바이트 그대로 복사하세요. HTML과 Markdown에는 각 학습 단계의 이미지가
사용자 학습 흐름 순서대로 실제 표시되어야 하며 파일명만 적어두면 안 됩니다.
...
- HTML의 `images/...` 상대 경로 이미지는 템플릿의 `inline_local_images()`로 Base64 데이터 URI로 변환한 뒤 CF_HTML에 넣으세요.
- 이를 통해 로컬 경로를 노출하지 않으면서 Notion 붙여넣기 데이터에 이미지 자체를 포함하세요.
...
- 모든 `img src="images/..."`와 Markdown 이미지 참조가 ZIP의 실제 이미지 파일을 가리키는지 검사하세요.
- 최소 1개 이상의 학습 캡처가 HTML의 `img` 태그와 `images/` 폴더에 포함됐는지 검사하세요.
...
- 최종 ZIP에는 루트 파일 `notion_ready.html`, `notion_ready.md`, `COPY_TO_NOTION.bat`, `copy_to_notion.py`, `README.txt`와 실제 사용한 캡처 원본의 `images/` 폴더를 포함합니다.
- HTML은 각 학습 단계에 `images/...` 상대 경로의 실제 이미지를 표시하고, 복사 도구는 이를 Base64 데이터 URI로 바꿔 CF_HTML에 포함합니다.
