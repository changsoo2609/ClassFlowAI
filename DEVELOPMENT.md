...
실제 API를 호출하지 않고 가상 카드와 Mock 응답으로 평가 구조, 오류 복구와 UI 비동기 흐름을 확인한다.

```powershell
cd _runtime
py -3.12 -c "import modules.study_answer_evaluator; print('STUDY_ANSWER_EVALUATOR_IMPORT_OK')"
py -3.12 -m unittest discover -s tests -p "test_study_answer_evaluator.py" -v
py -3.12 -m unittest discover -s tests -p "test_study_review_ai_ui.py" -v
```

실제 NVIDIA 모델 연결 검증 전에는 평가 결과를 품질 기준으로 사용하지 않는다. API 키, 전체 프롬프트·응답과 실제 학습자료는 테스트 또는 저장소에 남기지 않는다.
...
- 수업 작업공간의 `study/` 카드, 가져오기 이력, 근거 이미지, 복습 상태와 AI 평가 이력
