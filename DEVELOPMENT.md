...
1. 루트의 `INSTALL_FIRST.bat`을 실행한다.
2. 설치 완료 후 `START_HERE.bat`을 실행한다.
3. CMD 진행 표시가 실행 완료가 될 때까지 기다린다.
4. 프로그램 설정에서 개인 NVIDIA API 키와 저장 위치를 입력한다.
...
6. OCR 캡처 후 원문 즉시 표시, 자동 보정 및 보정본 단일 복사 확인
   - 보정 실패 시 원문이 자동 복사되지 않고 다시 시도·원문 수동 복사가 제공되는지 확인
   - `결과 다시 수정`이 입력 모달 없이 원본 이미지와 최신 결과로 재추론되는지 확인
   - 재추론 실패·저장 실패·수업 전환·오래된 응답에서 기존 정상 결과가 유지되는지 확인
   - 기존 OCR/CAP 작업과 상호 배제되고 직전 결과 복원이 원본 메타데이터를 바꾸지 않는지 확인
7. OCR 텍스트가 먼저 표시되고 수업 흐름 해석이 백그라운드에서 완료되는지 확인
8. CAP 캡처 후 원본 이미지 클립보드 유지 확인
9. CAP 결과 수동 복사 확인
10. 캡처를 위·아래로 이동한 뒤 목록과 공통 수업 흐름의 순서가 일치하는지 확인
   - 원래 촬영 순서 복원 취소/확인과 재실행 후 `display_order` 유지 확인
   - 정렬 전후 이미지 파일명, `captured_at`/`created_at`, `record_id`와 OCR/CAP 결과가 동일한지 확인
   - 그림과 바로 뒤 해설의 순서가 유지되는지 확인
   - 그림 우클릭은 원본 이미지, 해설·코드 우클릭은 해당 텍스트만 복사하는지 확인
   - 이미지가 누락되면 오류 블록을 표시하고 성공 메시지를 표시하지 않는지 확인
11. 전체 `unittest`, 전체 Python 컴파일과 `app.py` import 확인
12. 미니 상태창 우클릭 메뉴, 종료 취소와 확인 동작 점검
...
- 사용자 캡처 이미지 폴더
- 사용자 기록 JSON과 이벤트 기록
- 수업별 `YYYY-MM-DD/HH-MM-SS/` 폴더와 `.classflow_current_lesson.json` 포인터
- 기존 `lessons/lesson_날짜_시간` 수업 폴더 읽기 호환
...
사용자에게 전달하는 루트에는 다음 파일과 폴더가 보이도록 한다.
...
`_runtime/`에는 실행 코드, 모듈, requirements와 PowerShell 실행·설치 스크립트가 들어간다. 다음 항목은 배포본에 포함하지 않는다.
...
커밋 전에 `config.json`, 로그, 캡처와 기록 JSON이 staged 목록에 없는지 확인한다.
